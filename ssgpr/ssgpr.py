"""
This module implements the batch and incremental Sparse Spectrum Gaussian 
Process for Regression. A detailed description of the batch algorithm can be 
found in:
...
import numpy
import itertools
from scipy.linalg.lapack import dpotrf, dgetrs, dpotrs
from scipy.optimize import minimize
from scipy.linalg import cho_solve

from .chollrup import choluprk1

#from numpy.core.umath_tests import inner1d
inner1d = lambda a, b: numpy.einsum('...i,...i', a, b)
...
class SparseSpectrumFeatures(object):
    ...
    def __init__(self, n, nproj=50, sigma_o=0.1, sigma_o_prior=NoPrior(), l=[], l_prior=None, fixed=True):
...
class LinearGPR(object):
    ...
    def reset(self):
        ...
        self.L = numpy.asfortranarray(self.sigma_n * numpy.identity(n))
    ...
    def predict(self, Xstar):
        ...
        if Xstar.ndim == 2:
            ...
            vT, info = dgetrs(self.L, numpy.arange(self.L.shape[0]), 
    ...
    def train(self, X, Y):
        ...
        self.LPhiY, info = dgetrs(self.L, numpy.arange(self.L.shape[0]),
    ...
    def lmlgradient(self):
        ...
        LPhi, info = dgetrs(self.L,numpy.arange(self.L.shape[0]),
    ...
    def optimize(self, X, Y, solver='CG', ftol=None, gtol=None, maxIter=1000, 
                 maxFunEvals=None, checkgrad=False, verbose=False):
        ...
        if verbose:
            ...
            symboliterator = itertools.cycle(['-','\\','|','/'])
            def itercb(x, *args):
                fval = f(x)
                sys.stdout.write('{:40s}\r'.format('{} iteration: {:4d} lml: {:g}'.format(next(symboliterator), next(itercounter), fval)))

        else:
            itercb = lambda *args: None
        ...
        r = minimize(f, x0, method=solver, jac=df, options=options, callback=itercb)

        self.setparams(convert(r.x))

        if verbose:
            sys.stdout.write('{:40s}\r'.format(''))

        self.reset()
        return r 


def SSGPR(n, p, nproj=50, fixed=True):
    """Convenience function to easily construct a standard SSGPR."""
